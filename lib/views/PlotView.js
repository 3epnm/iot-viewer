var PlotView = (function () {
	...
	var updateRangeLegend = function () {
		...
		var from = (new Date(axes.xaxis.min)).getFormatDateTime();
		var to = (new Date(axes.xaxis.max)).getFormatDateTime();
		$(".plotfrom").text(from);
		$(".plotto").text(to);		
	}
	...
	var init_plot = function () {
		...
		if (AppConfig.get("pan")) {
			events.pan =  $(selector).bind("plotpan", function (event, plot) {
				...
				var data = instance_plot.getData();
				var pan = {
					set : {
						min : new Date(parseInt(axes.xaxis.min)),
						max : new Date(parseInt(axes.xaxis.max))
					}, 
					is : {
						min: AppConfig.getFrom(),
						max: AppConfig.getTo()
					}
				};
				...
				if (is_updating || Math.abs( (pan.is.min.getTime() - pan.set.min.getTime()) / 1000 ) < 1000) {
					return;
				}
				
				var duration = new Date(pan.is.max.getTime() - pan.is.min.getTime());
				var set = {
					from: pan.set.min,
					to: pan.set.max
				};
				if (pan.set.min < pan.is.min && pan.set.max <= pan.is.max) {
					set.load_from = new Date(pan.is.min - duration);
				}
			 	if (pan.set.min >= pan.is.min && pan.set.max > pan.is.max) {
			 		set.load_to = new Date(pan.is.max + duration);
				}
				AppConfig.save(set, {
